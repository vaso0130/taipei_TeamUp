...
2. `你來對地方了。`
3. `快來看看誰正在等你來組隊`
4. **`台北配`** ＋ 小字 `配.taipei`（品牌字與內文同為黑體 Noto Sans TC；2026-09-09 依負責人要求取消楷體）
...
- 沒有 open：顯示 `目前沒有進行中的活動`＋ closed 活動的折疊區。
- closed 活動預設摺疊。`closed` 只代表停止招募，所以每張卡依 `endsAt` 標 `已結束`（已過）或 `已停止招募`（未過）；摺疊區標題依內容命名——全部已結束為 `已結束的活動（n）`，否則 `已停止招募或已結束的活動（n）`。仍可點進去看。
- archived 不出現在任何列表（公開 API 不回傳，前端亦只收 open／closed）。
...
- closed：活動首頁 hero 下方灰色提示——`endsAt` 已過為 `這場活動已結束`，未過為 `這場活動已停止招募`；建立／申請按鈕消失（既有 `recruitOpen` 邏輯），瀏覽仍可。
