...
- 卡片式（不是表格）：每張卡＝一場活動。左上狀態徽章（草稿／招募中／已停止招募／已封存），標題＝活動名稱，副標＝slug（等寬字、灰）。
- 三個數字一行：活動日期區間、招募截止、`隊伍 n・參加者 n`（術語走該活動自己的 termTeam）。
- 動作列兩行：第一行 **編輯**（主要）、複製為新活動、匯出 JSON；第二行狀態動作（依目前狀態顯示唯一的下一步：開放招募／停止招募／封存（從列表隱藏）；已停止招募另有「重新開放招募」）與「刪除活動」（counts 皆 0 才可按，否則 disabled 並 tooltip「已有參加者或{termTeam}，只能封存」）。所有動作走與編輯器相同的確認框與文案（`lib/event-status.ts`），成功後重新載入並以 `role=status` 播報。（2026-09-09 修訂：原本狀態切換只放編輯器，營運者反映在列表找不到。）
- 排序：招募中 → 草稿 → 已停止招募 → 已封存；同組內依開始時間。已封存預設摺疊在「顯示已封存（n）」之後。
...
狀態文案（前後台一致）：draft「草稿」、open「招募中」、closed「已停止招募」、archived「已封存」。`closed` 只代表停止招募——活動本身可能還沒開始；公開端對 closed 依 `endsAt` 判讀為「已結束」或「已停止招募」（見 landing spec §1、§4）。

狀態卡（區塊 ⑧）內容：目前狀態徽章＋一段「參加者現在看到什麼」的說明；下一步按鈕一顆，次要動作以文字連結呈現：

| 目前 | 主要動作 | 說明文字 | 確認對話框內容 |
|---|---|---|---|
| draft | 開放招募 | 「草稿對外不可見。開放招募後會出現在活動列表，參加者可以填資料、開團。」 | 開放前檢查清單（伺服端回傳）：名稱、三個時間有效、招募截止在未來、人數規則、≥1 角色、≥1 技能。有缺就列出並禁止。 |
| open | 停止招募 | 「參加者可以開團、申請、傳訊息。」 | 「停止後不能再開{termTeam}與申請，既有{termTeam}與訊息仍可見；活動仍會出現在列表的摺疊區。可以重新開放招募。」 |
| closed | 封存（從列表隱藏） | 「不能開團與申請；仍會出現在活動列表的摺疊區，資料依保存天數清除。」次要：重新開放招募 | 「封存後從所有列表隱藏，直接連結仍可瀏覽。此動作不可逆。」 |
| archived | （無） | 「已從所有列表隱藏；直接連結仍可瀏覽。」 | — |

狀態變更是獨立 API（不是隨儲存一起送），有未儲存變更時按鈕 disabled 並提示「先儲存變更」。變更成功的 `role=status` 播報在下一次儲存或狀態變更前清空。

刪除活動：**任何狀態，只要沒有任何參加者／隊伍** 即可刪（2026-09-09 放寬，原限草稿），按鈕「刪除活動」放在狀態卡最下方紅色文字連結，確認框說明「會刪除這場活動的設定與字典。此動作不可逆。」；有資料的活動不提供刪除，只有封存（提示「已有參加者或{termTeam}，只能封存」）。
...
| DELETE | `/api/admin/events/:slug` | 任何狀態，counts 皆 0 → 204；否則 409 `event_not_empty` `{ teams, participants }`。 |
